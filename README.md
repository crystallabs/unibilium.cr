...
Additional convenience functions by the Crystal binding are:
...
determined from a string, there is no single `#get`/`#get?` method; instead there are the individual
...
Convenience functions existing on top of basic bindings:
...
The complete list can be found or modified in `support/tables.cr`; its current version is copied here:
